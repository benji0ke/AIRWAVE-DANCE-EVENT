This  event site  is created for Airwave Entertainement.  When a user enters [https://benji0ke.github.io/Airwave-Dance-Event/](https://benji0ke.github.io/Airwave-Dance-Event/) in a browser, the browser breaks the URL into parts — scheme (`https`), hostname (`benji0ke.github.io`), and path (`/Airwave-Dance-Event`). It then asks the DNS resolver for the server’s IP address. The resolver follows the DNS chain (root → TLD → authoritative) until it finds the correct IP and sends it back.
